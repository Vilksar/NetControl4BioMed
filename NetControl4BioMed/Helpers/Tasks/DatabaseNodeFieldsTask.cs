        ...
        public IEnumerable<DatabaseNodeField> Create(IServiceProvider serviceProvider, CancellationToken token)
        {
            ...
            for (var index = 0; index < count; index++)
            {
                ...
                foreach (var batchItem in batchItems)
                {
                    ...
                    {
                        // Throw an exception.
                        throw new ArgumentException($"A database node field with the name \"{batchItem.Name}\" already exists.");
                    }
                    // Check if there was no database provided.
                    if (batchItem.Database == null || string.IsNullOrEmpty(batchItem.Database.Id))
                    {
                        // Throw an exception.
                        throw new ArgumentException($"There was no database provided for the database node field \"{batchItem.Name}\".");
                    }
                    // Get the database.
                    ...
                    {
                        // Throw an exception.
                        throw new ArgumentException($"There was no database found for the database node field \"{batchItem.Name}\".");
                    }
                    // Check if the database is generic.
                    if (database.DatabaseType.Name == "Generic")
                    {
                        // Throw an exception.
                        throw new ArgumentException($"The database node field \"{batchItem.Name}\" can't be generic.");
                    }
                    // Define the new item.
        ...
        public IEnumerable<DatabaseNodeField> Edit(IServiceProvider serviceProvider, CancellationToken token)
        {
            ...
            for (var index = 0; index < count; index++)
            {
                ...
                foreach (var batchItem in batchItems)
                {
                    ...
                    {
                        // Throw an exception.
                        throw new ArgumentException("The generic database node field can't be edited.");
                    }
                    // Check if there is another database node field with the same name.
