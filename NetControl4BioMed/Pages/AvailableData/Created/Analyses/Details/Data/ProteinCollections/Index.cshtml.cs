...
namespace NetControl4BioMed.Pages.AvailableData.Created.Analyses.Details.Data.ProteinCollections
{
    public class IndexModel : PageModel
    {
        ...
        public class ViewModel
        {
            ...
            public SearchViewModel<AnalysisProteinCollection> Search { get; set; }
        ...
        public async Task<IActionResult> OnGetAsync(string id, string searchString = null, IEnumerable<string> searchIn = null, IEnumerable<string> filter = null, string sortBy = null, string sortDirection = null, int? itemsPerPage = null, int? currentPage = 1)
        {
            ...
            query = query
                .Where(item => input.Filter.Contains("IsSource") ? item.Type == AnalysisProteinCollectionType.Source : true)
                .Where(item => input.Filter.Contains("IsNotSource") ? item.Type != AnalysisProteinCollectionType.Source : true)
                .Where(item => input.Filter.Contains("IsTarget") ? item.Type == AnalysisProteinCollectionType.Target : true)
                .Where(item => input.Filter.Contains("IsNotTarget") ? item.Type != AnalysisProteinCollectionType.Target : true);
            ...
            View = new ViewModel
            {
                Analysis = items
                    .First(),
                Search = new SearchViewModel<AnalysisProteinCollection>(_linkGenerator, HttpContext, input, query)
            };
            // Update the view.
