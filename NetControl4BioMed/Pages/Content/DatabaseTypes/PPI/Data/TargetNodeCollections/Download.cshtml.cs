...
namespace NetControl4BioMed.Pages.Content.DatabaseTypes.PPI.Data.TargetNodeCollections
{
    public class DownloadModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(IEnumerable<string> ids)
        {
            ...
            {
                // Display a message.
                TempData["StatusMessage"] = "Error: No target protein collections have been found with the provided IDs, or you don't have access to them.";
                // Redirect to the index page.
                return RedirectToPage("/Content/DatabaseTypes/PPI/Data/TargetNodeCollections/Index");
            }
            // Return the page.
            ...
            return new FileCallbackResult(MediaTypeNames.Application.Zip, async (zipStream, _) =>
            {
                ...
                if (Input.FileFormat == "Text")
                {
                    ...
                    foreach (var nodeCollection in View.Items)
                    {
                        ...
                        using var stream = archive.CreateEntry($"TargetProteinCollection-{nodeCollection.Name.Replace(" ", "-")}-{nodeCollection.Id}.txt", CompressionLevel.Fastest).Open();
                ...
                else if (Input.FileFormat == "Json")
                {
                    ...
                    {
                        // Create a new entry in the archive and open it.
                        using var stream = archive.CreateEntry($"TargetProteinCollection-{nodeCollection.Name.Replace(" ", "-")}-{nodeCollection.Id}.json", CompressionLevel.Fastest).Open();
                        // Get the required data.
                        var data = new
                        {
                ...
                else if (Input.FileFormat == "Excel")
                {
                    ...
                    {
                        // Create a new entry in the archive and open it.
                        using var stream = archive.CreateEntry($"TargetProteinCollection-{nodeCollection.Name.Replace(" ", "-")}-{nodeCollection.Id}.xlsx", CompressionLevel.Fastest).Open();
                        // Get the required data.
